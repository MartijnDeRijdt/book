...
document.getElementById("filterPrice").addEventListener("click", async () => {
    ...
    await fetchBooks(`http://localhost:8080/api/book/booksWithPriceMoreThen?price=${search}`);
    renderBooks(books);
})

document.getElementById("filterTitle").addEventListener("click", async () => {
    const search = document.getElementById("title").value;
    const result = await (await fetch(`http://localhost:8080/api/book/bookWithTitle/${search}`)).json();
    books.length = 0;
    books.push(result);
    renderBooks(books);
})
...
document.getElementById("color").addEventListener("click", async () => {
    if (document.getElementById("color").checked) {
        await fetchBooks("http://localhost:8080/api/book/booksInColor");
    }
    else {
        await fetchBooks("http://localhost:8080/api/book/all");
    }
